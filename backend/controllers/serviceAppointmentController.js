  ...
  try {
    ...
    try {
      const created = await ServiceAppointment.create({
        ...base,
        status: "Confirmed",
        payment: { method: "NotApplicable", status: "Pending", amount: numericAmount, sessionId: session.id || "" },
      });
...
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    if (updates.payment) {
      ...
      if (method && String(method).toLowerCase() === "NotApplicable") updates.status = updates.status || "Confirmed";
